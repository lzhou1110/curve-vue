<template>
	<div class="add-liquidity">
                ...
                <li v-for='(currency, i) in Object.keys(currencies)'>
                    <label :for="'currency_'+i" class='currency_label'>
                        ...
                        <span v-show='withdrawc'>{{currencies[currency]}}
	                    	<span v-show="!(currency == 'usdt' && currentPool == 'usdt') && !['susdv2', 'ren', 'sbtc', 'hbtc', '3pool'].includes(currentPool)">(in {{currency | capitalize}})</span>
                    	</span>
                    	<span v-show="!withdrawc && !['susdv2', 'tbtc', 'ren', 'sbtc', 'hbtc', '3pool'].includes(currentPool)">{{currency | capitalize}}</span>
                        <span v-show="!withdrawc && ['susdv2', 'tbtc', 'ren', 'sbtc', 'hbtc', '3pool'].includes(currentPool)">{{currencies[currency]}}</span>
                    </label>
                    <input type="text" 
                    :id="'currency_'+i" 
                    name="from_cur" 
        ...
        <fieldset v-show = "currentPool != 'susd'">
        	...
        	<ul>
        		<li v-show = "!['susdv2','tbtc','ren', 'sbtc', 'hbtc', '3pool'].includes(currentPool)">
	            ...
	            <li>
	            	...
	            	<label 
                        for='donate_dust' 
                        v-show="!['tbtc', 'ren', 'sbtc', 'hbtc', '3pool'].includes(currentPool)">Donate dust
...
<script>
    ...
    export default {
        ...
        methods: {
            async mounted() {
            	if(['susdv2', 'tbtc', 'ren', 'sbtc', 'hbtc', '3pool'].includes(this.currentPool)) {
            		this.withdrawc = true;
			...
			async handle_remove_liquidity(unstake = false, unstake_only = false, exit = false) {
                ...
                if(['tbtc','ren', 'sbtc', 'hbtc', '3pool'].includes(currentContract.currentContract)) inOneCoin = currentContract.swap
			    ...
			    if (this.share == '---') {
			        ...
			        if(this.withdrawc || ['susdv2', 'sbtc'].includes(this.currentPool)) {
                        ...
                        try {
                            ...
                            if(!['tbtc','ren','sbtc', 'hbtc', '3pool'].includes(currentContract.currentContract)) await common.ensure_allowance_zap_out(token_amount, undefined, undefined, this.inf_approvalamount)
                            dismiss()
                            this.waitingMessage = 'Please confirm withdrawal transaction'
                            var { dismiss } = notifyNotification(this.waitingMessage)
                            await helpers.setTimeoutPromise(100)
                    ...
                    if(this.to_currency !== null && this.to_currency < 10) {
                        ...
                        var { dismiss } = notifyNotification(this.waitingMessage)
                        this.estimateGas = contractGas.depositzap[this.currentPool].withdraw / 2
                        if(!['tbtc','ren','sbtc', 'hbtc', '3pool'].includes(currentContract.currentContract)) await common.ensure_allowance_zap_out(amount, undefined, undefined, this.inf_approval)
                        dismiss()
                        let min_amount;
                        try {
                        ...
                        if(!['tbtc','ren','sbtc', 'hbtc', '3pool'].includes(currentContract.currentContract)) args.push(this.donate_dust)
                        await helpers.setTimeoutPromise(100)
			        	try {
